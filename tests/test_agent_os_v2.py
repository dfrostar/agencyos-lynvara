...
class TestAgentOSStore:
    ...
    def test_increment_signal_count_does_not_deadlock(self, tmp_path):
        """increment_signal_count must return without hanging.

        Regression for CRITICAL finding N1: store.py:517 calls get_proposal() while
        holding the non-reentrant self._lock inside _tx(), which deadlocks on a
        second signal to an open proposal (auto_trigger.py:93). Run under a
        watchdog thread so a regression fails the test instead of hanging pytest.
        """
        import threading
        ...
        prop = store.create_proposal(
            "t", "title", "hyp", "baseline", "candidate", "latency_ms", 100.0, 90.0
        )
        store.audit("t", "alice", "created", prop["proposal_id"])
        ...
        def worker():
            try:
                result["val"] = store.increment_signal_count("t", prop["proposal_id"])
            except Exception as exc:  # pragma: no cover - exercise happens above
                err["exc"] = exc
        ...
        t.join(timeout=5.0)
        if t.is_alive():
            # The worker is blocked re-acquiring the non-reentrant lock.
            raise AssertionError(
                "increment_signal_count deadlocked (re-acquires non-reentrant lock)"
            )
        if "exc" in err:  # pragma: no cover - should not raise
            raise err["exc"]
